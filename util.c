...
double hinGetDifficulty(unsigned int nBits, void* someObject)
{
	if(someObject == NULL)
	{
		return 1.0;
...
double lastRDifficulty = 0;
int lastRHeight = 0;
double retargetN = 10;
double retargetR = 1;
unsigned char hin_getNFactor(const long int nHeight, double diff)
{	
	if(nHeight == 0)
	{
		lastDifficulty = diff;
	}
	if(nHeight - lastNHeight == 20)
	{
		double alpha = 0.075;
		retargetN = retargetN + alpha * (diff - lastDifficulty);
		lastDifficulty = diff;
		lastNHeight = nHeight;
	}
	int casted = (int) retargetN;
	return (unsigned char) casted;
}

int hin_getRFactor(const long int nHeight, double diff)
{
	if(nHeight == 0)
	{
		lastRDifficulty = diff;
	}
	if(nHeight - lastRHeight == 3360)
	{
		double alpha = .003;
		retargetR = retargetR + alpha ( diff - lastRDifficulty);
	}
	int cast = (int) retargetR;
	if(cast < 1) return 1;
	return cast;
}
